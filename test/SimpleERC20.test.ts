...
const setup = deployments.createFixture(async (_, name?: string) => {
  await deployments.fixture(name || 'SimpleERC20');
  const {simpleERC20Beneficiary} = await getNamedAccounts();
  const contracts = {
    SimpleERC20: <IERC20>await ethers.getContract(name || 'SimpleERC20'),
  };
...
describe('TestERC20', function () {
  it('transfer fails', async function () {
    const {users} = await setup('TestERC20');
    await expect(
      users[0].SimpleERC20.transfer(users[1].address, 1)
    ).to.be.revertedWith('NOT_ENOUGH_TOKENS');
  });
  ...
  it('transfer succeed', async function () {
    const {users, simpleERC20Beneficiary} = await setup('TestERC20');
    await simpleERC20Beneficiary.SimpleERC20.transfer(users[1].address, 1);
  });
});
